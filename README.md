...
- **Dead-lettered** means a batch failed in a way that retrying it can never
  fix (Loki answering `400`, `413`, or `422`, for example), so it was moved
  out of the live queue into `/spool/<sink>/dead/` rather than blocking
  everything behind it. Nothing deletes these automatically; find and clear
  them by hand.
